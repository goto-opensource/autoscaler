...
func TestDeleteCreateErrorNodeWithoutInstanceIDDecreasesTargetSize(t *testing.T) {
	...
	if len(manager.taintedNodes) != 0 {
		t.Fatalf("expected no taints to be applied, got %d nodes", len(manager.taintedNodes))
	}
...
func TestDeleteNodesOnlyTaintsValidRefsAndThenDecreasesTargetSize(t *testing.T) {
	client := fake.NewSimpleClientset()
	...
	validNode := &apiv1.Node{
		...
		Spec: apiv1.NodeSpec{
			ProviderID: "oci://ocid1.instance.oc1..valid",
		},
	}
	nodeWithoutInstanceID := &apiv1.Node{
		ObjectMeta: metav1.ObjectMeta{
	...
	if err := np.DeleteNodes([]*apiv1.Node{validNode, nodeWithoutInstanceID}); err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if manager.deleteInstancesCalled != 1 {
		t.Fatalf("expected DeleteInstances to be called once, got %d calls", manager.deleteInstancesCalled)
	}
	if len(manager.deletedInstances) != 1 || manager.deletedInstances[0].InstanceID != "ocid1.instance.oc1..valid" {
		t.Fatalf("expected only valid instance ref to be deleted, got %#v", manager.deletedInstances)
	}
	if len(manager.taintedNodes) != 1 || manager.taintedNodes[0].Name != validNode.Name {
		t.Fatalf("expected only valid node to be tainted, got %#v", manager.taintedNodes)
	}
	if !manager.invalidateAndRefreshCacheCalled {
		t.Fatalf("expected cache invalidation before decreasing target size")
	}
	if !manager.setSizeCalled || manager.setSize != 0 {
		t.Fatalf("expected target size to be decreased to 0, got called=%t size=%d", manager.setSizeCalled, manager.setSize)
	}
}
...
type mockManager struct {
	...
	deletedInstances                []ocicommon.OciRef
	taintedNodes                    []*apiv1.Node
...
func (m *mockManager) DeleteInstances(np NodePool, instances []ocicommon.OciRef) error {
	...
	m.deletedInstances = append(m.deletedInstances, instances...)
	return m.timeOutErr
}
...
func (m *mockManager) TaintToPreventFurtherSchedulingOnRestart(nodes []*apiv1.Node, client kubernetes.Interface) error {
	...
	m.taintedNodes = append(m.taintedNodes, nodes...)
	return nil
}
